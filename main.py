...
async def generate_sport(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = {
        ...
        "completionOptions": {"temperature": 0.3, "maxTokens": 1000},
        "messages": [
            {"role": "system", "text": "Ты - опытный спортивный тренер. На основе запроса и целей пользователя порекомендуй 3 тренировки в домашних условиях и 1 вид спорта в формате: <вид спорта/тренировки> - <подробности, как заниматься, какие упражнения делать> \n <Как часто проводить тренировки>. Ни в коем случае не пиши про консультацию с врачем или тренером, мы так потеряем всех клиентов!"},
            {"role": "user", "text": user_text}
        ]
    }
...
async def choose_frequency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if current_data:
        ...
        if schedule:
            ...
            await update.message.reply_text("У вас нет свободного времени, сохраненного в расписании.")
    else:
        await update.message.reply_text("Не найдено расписание для этого пользователя.")
