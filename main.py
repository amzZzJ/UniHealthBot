...
async def generate_food(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = {
        ...
        "messages": [
            {"role": "system", "text": "Ты - опытный повар-диетолог. Напиши 2 рецепта завтрака, 2 рецепта обеда, 2 рецепта ужина, с учетом целей и запросов пользователя. Пиши без воды. Продукты должны быть доступные для всех. Также пиши рецепт приготоваления."},
...
async def generate_sport(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = {
        ...
        "completionOptions": {"temperature": 0.3, "maxTokens": 1000},
        "messages": [
            {"role": "system", "text": "Ты - опытный спортивный тренер. На основе запроса и целей пользователя порекомендуй 2 тренировки в домашних условиях и 1 вид спорта в формате: <вид спорта/тренировки> - <подробности, как заниматься, какие упражнения делать> \n <Как часто проводить тренировки>."},
            {"role": "user", "text": user_text}
        ]
    }
