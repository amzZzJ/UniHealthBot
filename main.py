...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Привет! Я - бот, который поможет вам контролировать! Я много чего умею, весь список команд можете увидеть в меню!\n "
                                    "Для начала советую установить свое свободное время с помощью команды /set_schedule\n"
                                    ...
                                    "    * Команда /generate_sport поможет вам подобрать тренировки, а также выбрать расписание для них. Затем вы можете установить еженедельные напоминания (используйте команду /set_reminder и /delete_reminder для добавления и удаления напоминаний)\n"
                                    ...
                                    "P.S. Генерация тренировок и рецептов может быть долгой, не пугайтесь! Просто я пытаюсь выбрать лучшее для вас!")
    ...
    if scheduler.get_job(job_id) is None:
        ...
        scheduler.add_job(
            ...
            id=job_id,
            replace_existing=True,  # Заменяет, если уже существует задача с таким ID
            args=[context, chat_id, "Доброе утро! Не забудь сгенерировать рецепты на сегодня!"]
        )
        print(
...
async def choose_frequency(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if current_data:
        ...
        if schedule:
            ...
            await query.message.reply_text("У вас нет свободного времени, сохраненного в расписании.", reply_markup=ReplyKeyboardRemove())
            return ConversationHandler.END
    else:
        await query.message.reply_text("Не найдено расписание для этого пользователя.", reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
